...
from .constants import SQL_WRITE_BLACKLIST, SWAGGER_JSON_TEMPLATE, SWAGGER_DICT
from .jinjasql_loader import configure_jinjasql

from .serializers import ChartSerializer, FilterSerializer
...
class DatabaseView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not database_response:
            raise DatabaseConfigurationException('No databases defined')
    ...
    def _parse_params(self, params, parameter_definitions):
        for index, param in enumerate(parameter_definitions):
            ...
            if params.get(param.name):
                params[param.name] = parameter_type(param.name, **param.kwargs).to_internal(params[param.name])
        return params
...
class ChartsLoaderView(APIView):
    ...
    def post(self, request):
        ...
        if not data['id'] and Chart.objects.filter(url=data['url']).exists():
