...
from ..models import Chart,Transformation
...
class TransformersTestCase(BaseTestCase):
    ...
    def _create_schema_for_split(self):
        with connection.cursor() as c:
            ...
            for value in values_list:
                query1 = 'INSERT INTO employee_monthly_salary VALUES(' + _quoted_string(value) + ')'
                c.execute(query1)
    ...
    def test_transpose_transformation(self):
        chart = Chart.objects.create(
            ...
            database='default', transpose=True)
        response = self.client.get('/squealy/' + chart.name + '/')
    ...
    @skip("Split is not supported currently")
    def test_split_transformation(self):
        self._create_schema_for_split()
        chart = Chart.objects.create(url='test-split',
        ...
        transform_object = Transformation.objects.create(chart=chart)
        transform_object.name = 2
        transform_object.chart = chart
        transform_object.kwargs = {"metric_column": "salary", "pivot_column": "month"}
        transform_object.save()

        response = self.client.get('/squealy/' + chart.url + '/')
    ...
    @skip("Merge is not supported currently")
...
def _quoted_string(values):
    quoted_values = ["'" + str(value) + "'" for value in values]
